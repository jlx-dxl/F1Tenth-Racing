...
class ReactiveFollowGap(Node):
    ...
    def __init__(self):
        ...
        # Params
        self.declare_parameter('window_size', 9)   # size of window to mean-filt the scan
        self.declare_parameter('max_scan_threshold', 3.6)   # threshold to clip the scan
        self.declare_parameter('disparity_threshold', 0.27)   # threshold for selecting large enough disparities
    ...
    def preprocess_lidar(self, ranges):
        ...
        # reject high values
        max_scan_threshold = self.get_parameter('max_scan_threshold').get_parameter_value().double_value
        # print("max_scan_threshold:", max_scan_threshold)
        clipped_ranges = np.clip(ranges, 0, max_scan_threshold)
        # print("clipped_ranges:", clipped_ranges)
        
                # Average over window
        # print("ranges:", ranges)
        window_size = self.get_parameter('window_size').get_parameter_value().integer_value
        window = np.ones(window_size) / window_size
        filtered_ranges = ndimage.convolve1d(clipped_ranges, window, mode='nearest')   # using 1d convolution to implement a mean filter
        # print("filtered_ranges:", filtered_ranges)

        return filtered_ranges

    def find_disparities(self, ranges):
        """ 
        Find disparities in the LiDAR readings
        """
        disparity_threshold = self.get_parameter('disparity_threshold').get_parameter_value().double_value

        # calculate disparities
        disparities = np.diff(ranges, prepend=np.array([0]))

        # find large enough diparities
        indices = np.where(abs(disparities) > disparity_threshold)[0] - 1

        self.get_logger().info("Number of Disparities: %d" % len(indices))
