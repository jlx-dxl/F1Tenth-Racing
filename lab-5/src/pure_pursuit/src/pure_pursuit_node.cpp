...
private:
    ...
    void calculateClosestWaypoint() {
        ...
        for (size_t i = 0; i < waypoints.size(); ++i) {
            double distance = (waypoints[i] - currPos).norm(); // Euclidean distance
        ...
        Eigen::Vector2d closestPoint = waypoints[closestIndex];
    ...
    Eigen::Vector2d getClosestPointBeyondLookaheadDist(double threshold) {
    ...
    void pose_callback(geometry_msgs::msg::Quaternion quat)
    {
        ...
        Eigen::Vector2d targetPoint = getClosestPointBeyondLookaheadDist(L);
        ...
        drive_msg.drive.speed = 1.0; // Example fixed speed, adjust as necessary
