...
class LineGenerator:
    def __init__(self, corners):
        self.v_min = 4                      # the target speed at the end of braking
        self.v_max = 8                      # the max speed on the straights
        self.s_brake = 4                    # distance from the beginning of the turn to start braking
        self.s_turn = 0.23                  # distance down the road from the inner corner to begin turn
        self.s_straight = 2.0               # distance down the road from the exit of the inner corner to end turn
        self.coast_ratio = 0.4              # ?
        self.floor_it = 0.3                 # how quickly to speed up after coasting
        self.safe_dist = 0.5                # the distance to leave from the outer wall
        self.dist_straight_points = 0.3     # rough distance in meters between waypoints on the straight
        self.dist_brake_points = 0.3        # same as straight points but for braking
        self.num_turn_points = 10           # number of waypoints to have in turns
        self.corners = corners              

        self.inners = np.zeros((2, len(self.corners) + 1))
        self.outers = self.inners.copy()
        for i in range(len(self.corners)):
            self.inners[0, i] = self.corners[i]['inner'][0]
            self.inners[1, i] = self.corners[i]['inner'][1]
            self.outers[0, i] = self.corners[i]['outer'][0]
            self.outers[1, i] = self.corners[i]['outer'][1]
        self.inners[:, -1] = self.inners[:, 0]
        self.outers[:, -1] = self.outers[:, 0]

        self.num_brake_points = floor(self.s_brake / self.dist_brake_points)  # number of waypoints to have in braking zone
    ...
    def place_point_near_wall(self, turn_from, turn_to, dist, go_fwd):
        ...
        vec_buff_pre = self.safe_dist * unit_wall_outer  # buffer is still pointing along wall
        if go_fwd:
            rot = pi / 2
        else:
            rot = -pi / 2
    ...
    def def_turn(self, curr_turn_idx, start_vel, turn_finished):  # curr_turn: index from 0 to 3 of which turn you're looking at
        ...
        turn_now = self.corners[curr_turn_idx % len(self.corners)]
        turn_next = self.corners[(curr_turn_idx + 1) % len(self.corners)]
        turn_prev = self.corners[(curr_turn_idx - 1) % len(self.corners)]
...
racing_line = LineGenerator(corners)
...
with open(csvfile, mode='w', newline='') as file:
    ...
    writer.writerow(["x", "y", "v", "yaw"])  # Column headers
    for waypoint in np.array(racing_line.waypoints):
        print(waypoint)
