...
class WallFollow(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('steer_angle_threshold', 60)   # steering angle constrain
        self.declare_parameter('plot_switch', "off")   # debug tool, turn it on to see curves ("on" or "off")
        self.declare_parameter('forward_distance_threshold_1', 2.3)   # threshold of forward distance to help go through the gap and the northwest corner 
        self.declare_parameter('forward_distance_threshold_2', 6.6)   # threshold of forward distance to help determine if to steer
        self.declare_parameter('forward_distance_speed', 5.0)   # L in the slides
        self.declare_parameter('angle_clip_1', 0.05)   # if forward_distance > forward_distance_threshold, clip angle into this range, which means 
                                                     # when there is no wall in the front, too large angle is not allowed
        self.declare_parameter('angle_clip_2', 0.015)
        
        self.declare_parameter('high_speed_1', 9.0)   # speed when angle < 15
        self.declare_parameter('high_speed_2', 4.8)   # speed when angle < 15
    ...
    def get_error(self, range_data, dist):
        ...
        self.theta_increment = (theta / sample_num) * np.pi / 180
        alpha_list = []
        ...
        for i in range(sample_num):
            if which_wall == "left":
                a = self.get_range(range_data, np.pi/2 - self.theta_increment*i)
    ...
    def pid_control(self, forward_distance, error):
        ...
        fdt1 = self.get_parameter('forward_distance_threshold_1').get_parameter_value().double_value
        ac1 = self.get_parameter('angle_clip_1').get_parameter_value().double_value
        fdt2 = self.get_parameter('forward_distance_threshold_2').get_parameter_value().double_value
        ac2 = self.get_parameter('angle_clip_2').get_parameter_value().double_value
        ...
        if forward_distance > fdt1 and forward_distance < fdt2:   # if no wall forward, should never turn to much
            angle = np.clip(angle,-ac1,ac1)
        elif forward_distance > fdt2:
            angle = np.clip(angle,-ac2,ac2)
        ...
        speed = self.calculate_speed(angle, forward_distance)
    ...
    def calculate_speed(self, angle, forward_distance):
        """
        ...
        """
        hs1 = self.get_parameter('high_speed_1').get_parameter_value().double_value
        hs2 = self.get_parameter('high_speed_2').get_parameter_value().double_value
        ...
        fds = self.get_parameter('forward_distance_speed').get_parameter_value().double_value
        
        if np.abs(angle) < 5 / 180 * np.pi:
            if forward_distance > fds:
                speed = hs1
            else:
                speed = hs2
        elif np.abs(angle) >= 2 / 180 * np.pi and np.abs(angle) < 20 / 180 * np.pi:
            speed = ms
        else:
            speed = ls
        return speed
    ...
    def scan_callback(self, msg:LaserScan):
        ...
        for i in range(10):
            range_list.append(self.get_range(ranges, 0 - self.theta_increment*i))
            range_list.append(self.get_range(ranges, 0 + self.theta_increment*i))
        forward_distance = np.mean(range_list)
        angle, speed = self.pid_control(forward_distance, error) # TODO: actuate the car with PID
